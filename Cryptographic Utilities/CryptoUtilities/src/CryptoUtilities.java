...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        /*
         * If m isn't zero, we find the remainder of the two numbers by dividing
         * larger number over the smaller one. We use recursion until m is zero,
         * meaning the GCD has been found.
         */
        if (!m.isZero()) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        /*
         * To determine whether a number is even, we obtain the last digit of
         * the number and check whether it's even.
         */
        int num = n.divideBy10();
        //We can't change the value of n because it's a reference value
        n.multiplyBy10(num);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber div = new NaturalNumber2(n);

        /*
         * Once again, I will be using recursion to update the power mod. If the
         * number is raised to 0, n will always equal to 1.
         */
        if (p.isZero()) {
            n.transferFrom(one);

            /*
             * If the power is even, the number is raised by two and finds the
             * modulus by diving the number m.
             */
        } else if (isEven(p)) {
            p.divide(two);
            powerMod(n, p, m);
            ...
            /*
             * If it's odd, then we essentially do the same method for when it's
             * even. The function continues until the number is raised to 0.
             */
        } else {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber one = new NaturalNumber2(1);
        //don't change the value of n
        NaturalNumber copy = new NaturalNumber2(n);

        n.decrement();
        /*
         * We can use the previous powerMod method to updates n to p-th power
         * modulo m.
         */
        powerMod(w, n, copy);
        n.increment();
        ...
        /*
         * If the witness is a composite, then n (with high likelihood but not
         * certrainty) that n is prime
         */
        boolean prime = w.compareTo(one) != 0;
        return prime;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        /*
         * We will use 50 candidates to ensure the best outcome. We want to have
         * as many random witness candidates to check if these candidates are a
         * witness to n being compossite.
         */
        int candidates = 50;

        //if it's 3, then it's prime
        if (n.compareTo(three) <= 0) {
            ...
            // if it's odd, we use 50 random candidates and compare
        } else {
            copy.decrement();
            for (int i = 0; i <= candidates; i++) {
                NaturalNumber rand = randomNumber(copy);
                /*
                 *
                 */
                while (rand.compareTo(one) <= 0 || rand.compareTo(copy) >= 0) {
                    rand = randomNumber(n);
                }
                //checks whether a witness that n is compossite
                prime = !isWitnessToCompositeness(rand, n);
            }
        }
